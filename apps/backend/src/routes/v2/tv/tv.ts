...
API.get('/informacion', async (req: Request, res: Response) => {
    try {
        const [noticias, eventos] = await conPrisma(async (p) =>
            p.$transaction([
                // Consulta para eventos
                p.eventos.findMany({
                    where: {
        ...
        return res.status(200).json({ message: "OK", datos: { noticias, eventos } })
    } catch (error) {
        return res.status(500).json({ message: "Error en el servidor", datos: null })
    }
